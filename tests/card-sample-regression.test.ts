...
maybeResventTest("Resvent sample card preserves APAP config and metrics", async () => {
  ...
  assert.equal(prepared.machine.pressureMin, "8 cmH2O");
  assert.equal(prepared.machine.pressureMax, "10 cmH2O");
  assert.equal(prepared.machine.pressureRelief, "IPR: On 1");
  assert.equal(prepared.latestClinicalDayIso, "2026-08-07");
  assert.equal(metrics.dateRangeStart, "May 10, 2026");
  ...
  assertApprox(metrics.avgUsageHours, 8.2164, 0.02, "avg usage");
  assertApprox(metrics.avgAhi, 2.1042, 0.02, "avg AHI");
...
maybeResventTest("Resvent 60-day report tracks the machine summary conventions", async () => {
  ...
  assert.equal(metrics.dateRangeStart, "June 9, 2026");
  ...
  assertApprox(metrics.avgUsageHours, 8.2924, 0.02, "avg usage");
  assertApprox(metrics.avgAhi, 2.1184, 0.02, "avg AHI");
  assertApprox(metrics.avgResidualApneas, 1.7526, 0.02, "avg residual apneas");
  assertApprox(metrics.avgCentralApneas, 0, 0.001, "avg central apneas");
  assertApprox(metrics.avgReraIndex, 0.5507, 0.02, "avg RERA");
  assertApprox(metrics.avgLeak, 0, 0.02, "median leak");
  assertApprox(metrics.machine.pressureAvg ?? null, 7.5317, 0.02, "avg pressure");
  assertApprox(metrics.machine.pressure95th ?? null, 9.0617, 0.02, "95th pressure");
  assertApprox(metrics.maxLeak30m, 17.2049, 0.05, "30 min leak");
  assertApprox(metrics.maxLeak60m, 120, 0.05, "60 min leak");
